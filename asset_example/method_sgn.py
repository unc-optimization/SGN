...
SGN algorithm for the Asset Allocation example.
...
def asset_sgn(x0, in_data, prob_params, alg_params, print_interval=2.0):
	...
	while n_epochs < alg_params['max_epochs']:
		# calculate jacobian and function estimators
		jac_est = f_jac(A_mat, x_cur[:-1], x_cur[-1], beta=beta, bs=b_1)
		func_est = f_func(A_mat,  x_cur[:-1], x_cur[-1], beta=beta, bs=b_2)
